...
{
    public class Coding_Conversion_Binary : ICoder
    {
        public bool Insert
        {
            get { return true; }
        ...
        public string Name
        {
            get { return "From Binary (00000000-11111111)"; }
            set { }
        }
        ...
        public ParameterInfo[] RetrieveArguments()
        {
            return new ParameterInfo[] {
                new ParameterInfo("To", ParameterType.ComboBox, new string[] { "Hexadecimal", "ASCII", "Decimal", "Base64", "Octal" })
            };
        }
        ...
        private string Private_Decrypt(string pattern, int toIndex, out IFeedbackInfo feedback)
        {
            ...
            for (int i = 0; i < splits.Length; ++i)
            {
                if (!string.IsNullOrWhiteSpace(splits[i]))
                {
                    if (splits[i].Length > 8)
                    {
                        ...
                        for (int j = 0; j < splits[i].Length; j += 8)
                        {
                            int conversion = Convert.ToInt32(splits[i].Substring(j, Math.Min(8, splits[i].Length - j)), 2);
                    ...
                    {
                        int conversion = Convert.ToInt32(splits[i], 2);
                        conversion = Math.Min(255, Math.Max(0, conversion));
                        dataList.Add((byte)conversion);
                    }
                }
            ...
            if (toIndex == 0)
            {
                ...
                {
                    outString.Append(data[i].ToString("X").PadLeft(2, '0'));
                    if (i != (data.Length - 1))
                    { outString.Append(" "); }
                }
