...
{
    public class Coding_Conversion_HEX : ICoder
    {
        public bool Insert
        {
            get { return true; }
        ...
        public string Name
        {
            get { return "From Decimal (000-255)"; }
            set { }
        }
        ...
        public ParameterInfo[] RetrieveArguments()
        {
            return new ParameterInfo[] {
                new ParameterInfo("To", ParameterType.ComboBox, new string[] { "Hexadecimal", "ASCII", "Binary", "Base64", "Octal" })
            };
        }
            ...
            for (int i = 0; i < splits.Length; ++i)
            {
                if (!string.IsNullOrWhiteSpace(splits[i]))
                {
                    if (splits[i].Length > 3)
                    {
                        ...
                        {
                            int conversion = Convert.ToInt32(splits[i].Substring(j, Math.Min(3, splits[i].Length - j)), 10);
                    ...
                    {
                        int conversion = Convert.ToInt32(splits[i], 10);
                        conversion = Math.Min(255, Math.Max(0, conversion));
                        dataList.Add((byte)conversion);
                    }
                }
