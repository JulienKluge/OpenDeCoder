...
{
    public class Coding_Conversion_DEC : ICoder
    {
        public bool Insert
        {
            get { return true; }
        ...
        public string Name
        {
            get { return "From Hexadecimal (00-ff)"; }
            set { }
        }
        ...
        public ParameterInfo[] RetrieveArguments()
        {
            return new ParameterInfo[] {
                new ParameterInfo("To", ParameterType.ComboBox, new string[] { "Decimal", "ASCII", "Binary", "Base64", "Octal" })
            };
        }
            ...
            for (int i = 0; i < splits.Length; ++i)
            {
                if (!string.IsNullOrWhiteSpace(splits[i]))
                {
                    if (splits[i].Length > 2)
                    {
                        ...
                        for (int j = 0; j < splits[i].Length; j += 2)
                        {
                            int conversion = Convert.ToInt32(splits[i].Substring(j, Math.Min(2, splits[i].Length - j)), 16);
                    ...
                    {
                        int conversion = Convert.ToInt32(splits[i], 16);
                        conversion = Math.Min(255, Math.Max(0, conversion));
                        dataList.Add((byte)conversion);
                    }
                }
